...
class AnonymizePredict:
    def __init__(self, input_path: Path | str, model_dir: Path | str = "Matela7/AnonBert-ENR"):
    ...
    def _resolve_id2label(self, model: AutoModelForTokenClassification, local_dir: Path | None) -> dict[int, str]:
        if local_dir:
    ...
    def tokens_to_placeholder_text(self, tokens: Sequence[str], tags: Sequence[str]) -> str:
        ...
        while i < len(tokens):
            ...
            i += 1
        return self.detokenize(compressed)
    ...
    def write_conll(self, output_path: Path, tokens_with_tags: List[List[Tuple[str, str]]]) -> None:
    ...
    def anonymize_file(self,
        ...
        model.eval()
        id2label = self._resolve_id2label(model, local_model_dir)
        ...
        with input_path.open("r", encoding="utf-8") as src:
            for line in src:
                ...
                tags = self.predict_tags(tokenizer, model, tokens, id2label)
                conll_sequences.append(list(zip(tokens, tags)))
                anonymized_lines.append(self.tokens_to_placeholder_text(tokens, tags))
        ...
        if conll_sequences:
            self.write_conll(conll_path, conll_sequences)
    ...
    def anonimize_fakefill(
        self,
        anonymized_path: Path | str | None = None,
        fake_output_path: Path | str | None = None,
    ) -> Path:
        if anonymized_path is None:
            anonymized_path, _ = self.anonymize_file()
        else:
            anonymized_path = Path(anonymized_path)

        if fake_output_path is None:
            fake_output_path = anonymized_path.with_name(
                f"{anonymized_path.stem}_fake{anonymized_path.suffix}"
            )
        else:
            fake_output_path = Path(fake_output_path)

        text = ReadText(str(anonymized_path))
        result = text.FakeFillAll()

        with fake_output_path.open("w", encoding="utf-8") as dst:
            dst.write(result)

        print(f"✓ Zapisano tekst z fałszywymi danymi do: {fake_output_path}")
